# Mollie

Mollie ist ein Zahlungsdienstleister, der es Unternehmen ermöglicht, Zahlungen online zu akzeptieren. Mit Mollie können Sie verschiedene Zahlungsmethoden wie Kreditkarten, PayPal, SEPA-Lastschrift und viele andere integrieren. Mit **365 business Banking** können Sie Ihr Mollie-Konto direkt mit **Microsoft Dynamics 365 Business Central** verbinden, um Transaktionen automatisch abzurufen und in den Zahlungsabgleich einzubeziehen.

## Voraussetzungen

- Ein aktives Mollie Konto.

## Schritt-für-Schritt-Anleitung

1. Wählen Sie **Bankkonten** in der Suche von Microsoft Dynamics 365 Business Central aus.
2. Erstellen Sie ein neues Bankkonto oder wählen Sie ein bestehendes aus.
3. Wählen Sie in der **Bankkonto Karte**, unter **Aktionen** die Aktionsgruppe **Kontoverbindung** und **Verbinden…** aus.
   ![Verbindung starten](/assets/images/365-business-banking/connect-banking-provider.de-DE.png)
4. Wählen Sie aus der Liste **Mollie** aus.
5. Bestätigen Sie die Bestätigugsabfrage mit **Ja**.
6. Klicken Sie auf **Authentifizieren**, um eine Verbindung zwischen Mollie Connect und 365 business Banking herzustellen.
7. Folgen Sie den Anweisungen auf dem Bildschirm, um die Authentifizierung abzuschließen.
   ![Mollie Authentifizieren](/assets/images/365-business-banking/mollie-connect.de-DE.png)

## Kontoverbindung trennen

1. Wählen Sie **Bankkonten** in der Suche von Microsoft Dynamics 365 Business Central aus.
2. Wählen Sie das Bankkonto aus, dessen Verbindung Sie trennen möchten.
3. Wählen Sie in der **Bankkonto Karte**, unter **Aktionen** die Aktionsgruppe **Kontoverbindung** und **Verbindung aufheben...** aus.
   ![Verbindung trennen](/assets/images/365-business-banking/disconnect-banking-provider.de-DE.png)
4. Bestätigen Sie die Bestätigugsabfrage mit **Ja**.
